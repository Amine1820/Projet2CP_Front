...
const BesoinsListmembre = ({ besoins }) => {
  return (
        ...
        <div className="evenement-preview flex flex-row p-4 mt-4 w-full justify-between" key={besoin.id}>
          ...
          <div className="px-8 mr-4 py-12 w-120">
            ...
            <button  style={{ backgroundColor: '#2E3840' }}
              className="font-extrabold  cart-btn flex r transition duration-500 ease-in-out px-2 py-2 w-24 self-center ml-[30%]">Voir Plus</button>
              </div>
          </div>
          </div>): null}
          {(besoin.id % 2 === 0) ? (
...
export default BesoinsListmembre;
