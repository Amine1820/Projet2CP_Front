import './Don.css';
import img from '../../Front_assets/Donation_don Visiteur.png'
...
<div className="bg-blue-400 h-screen flex justify-center items-center">
  ...
  <div className="box-sizing border-box mx-auto flex flex-col justify-start items-center p-4 md:p-8 gap-4 text-red absolute w-11/12 sm:w-5/6 md:w-2/3 lg:w-1/2 xl:w-2/5 bg-red shadow-lg rounded-lg">
    <h2 className="text-center text-yellow-300  text-4xl font-nohemi mt-4 sm:mt-6 md:mt-8 lg:mt-10 xl:mt-12">Don</h2>
    <div className="relative w-full mb-4 flex justify-between items-center">
      ...
      Le numéro de votre Carte de débit        <FaRegCreditCard className="absolute right-56 top-1/2 transform -translate-y-1/2 text-gray-500" size={16} />
      </span>
    </div>
    ...
    <div className="relative w-full mb-4 flex justify-between items-center ">
      ...
      <input
        ...
        required
      />
      <span className="Wilaya  absolute bottom-3 right-[21%] text-gray-400  text-sm duration-500 pointer-events-none">
        Date d'expiration 
      </span>
    </div>
    <div className="relative w-full mb-4 flex justify-between items-center">
      ...
      <span className="Wilaya absolute bottom-3 left-7 text-gray-400 text-sm duration-500 pointer-events-none">
        Montant 
        <MdAttachMoney className="absolute right-14 top-1/2 transform -translate-y-1/2 text-gray-500" size={18} />
      </span>
      <input
        className="w-5/12 h-8 bg-transparent border-b-2 border-gray-300 focus:outline-none focus:border-blue-500 focus:text-black ml-2"
        type="text"
      ...
      <span className="Wilaya  absolute bottom-3 right-[27.5%] text-gray-400 text-sm duration-500 pointer-events-none">
        CVV/CVC
        <AiOutlineMail className="absolute right-16 top-1/2 transform -translate-y-1/2 text-gray-500" size={16} />
    ...
    <div class="flex flex-col justify-center items-center md:flex-row md:justify-center md:items-center p-4 md:p-8 gap-4 md:gap-8 w-full md:w-auto h-auto md:h-71 bg-gray-800 md:bg-transparent shadow-lg md:shadow-none rounded-lg md:rounded-none">
      <button className="flex justify-center items-center  bg-blue-500 rounded-xl shadow-md py-4 px-8 text-white text-xl font-bold text-yellow-300  uppercase tracking-wide cursor-pointer hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400 w-full sm:w-80 h-14">
      Faire une donation 
      </button>
    </div>
  </div>
